...
def generator_loss(logits_fake):
    ...
    labels = torch.ones(len(logits_fake)).type(dtype)
...
def ls_discriminator_loss(scores_real, scores_fake):
    ...
    loss = ((scores_real - 1).square() + scores_fake.square()).mean() * 0.5
...
def ls_generator_loss(scores_fake):
    ...
    loss = (scores_fake - 1).square().mean() * 0.5
...
def build_dc_classifier(batch_size):
    ...
    model = nn.Sequential(
      Unflatten(batch_size, 1, 28, 28), # N, C, H, W
      nn.Conv2d(1, 32, 5, 1),
      nn.LeakyReLU(0.01),
      nn.MaxPool2d(2, 2),
      nn.Conv2d(32, 64, 5, 1),
