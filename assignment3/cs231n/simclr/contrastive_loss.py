...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        sim_k_N = torch.tensor([sim(z_k_N, z) for z in out[np.arange(2*N)!=k+N]])
        l1 = -((sim(z_k, z_k_N)/tau).exp() / (sim_k/tau).exp().sum()).log()
        l2 = -((sim(z_k_N, z_k)/tau).exp() / (sim_k_N/tau).exp().sum()).log()
        total_loss += l1 + l2
