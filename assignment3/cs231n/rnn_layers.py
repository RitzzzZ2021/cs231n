...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for i in range(x.shape[1]):
      step_x = x[:,i,:]
      next_h, cache_t = rnn_step_forward(step_x, prev_h, Wx, Wh, b)
      prev_h = next_h
      h.append(next_h)
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    A = np.hsplit(prev_h.dot(Wh) + x.dot(Wx) + b, 4)
    i = sigmoid(A[0])
    f = sigmoid(A[1])
    o = sigmoid(A[2])
    g = np.tanh(A[3])
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    for i in range(x.shape[1]):
      step_x = x[:,i,:]
      next_h, next_c, cache_t = lstm_step_forward(step_x, prev_h, prev_c, Wx, Wh, b)
      prev_h = next_h
      prev_c = next_c
      h.append(next_h)
      cache.append(cache_t)
...
def lstm_backward(dh, cache):
    ...
    (N, T, H), (D, H4) = dh.shape, cache[0][9].shape
    dx = np.empty((N, T, D))
    dh0 = np.zeros((N, H))
    dc0 = np.zeros((N, H))
    dWx = np.zeros((D, H4))
    ...
    for i in range(T-1,-1,-1):
      dx_t, dh0, dc0, dWx_t, dWh_t, db_t = lstm_step_backward(dh[:, i] + dh0, dc0, cache[i])
      dx[:, i] = dx_t
      dWx += dWx_t
      dWh += dWh_t
      db += db_t
